...
WEBCAM_INDEX = 0
#FRAME_RATE = 10.0
SUB_FOLDER_NAME = str(datetime.datetime.now())[5:].replace(
    ' ', '-').replace(":", '-').replace('.', '-')[:-3]
...
class ReadFromWebcam(object):
    def __init__(self, max_framerate=10.0, webcam_idx=0):
...
class VideoCamera(object):
    def __init__(self):
        self.video = ReadFromWebcam()
    os.makedirs(path + folder)
    print("i am running")

    def get_frame(self, cnt_image):
        success = self.video.has_image()
        if success:
            image = self.video.read_image()
            filename = path + "/" + folder+"/"+int2str(cnt_image, 5) + ".jpg"
